		...
		ScrollTrigger.create({
			...
			onUpdate: (self) => {
				...
				if (self.progress <= 0.2) {
					const yProgress = self.progress / 1.5;
					gsap.set(headers[0], { y: `${yProgress * 50}%` });
					gsap.set(headers[2], { y: `${yProgress * -50}%` });
				} else {
					gsap.set(headers[0], { y: '50%' });
					gsap.set(headers[2], { y: '-50%' });
